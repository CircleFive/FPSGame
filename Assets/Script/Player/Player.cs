...
public class Player : MonoBehaviour {
    ...
    private bool descheck1 = false;

    public bool DESCHECK1
    {
        get { return descheck1; }
        set { descheck1 = value; }
    }
   ...
   public void Start()
    ...
    private void Move()
    {
        ...
        int m_mask = LayerMask.NameToLayer("Ground");
        RaycastHit hit;
        //ジャンプ / 重力処理
        //if (m_characterController.isGrounded)
        if (Physics.SphereCast(transform.position, 1f, -Vector3.up, out hit, 0.5f, m_mask))
        {
            Debug.Log("abc");
            if (jAnimbool)
            {
